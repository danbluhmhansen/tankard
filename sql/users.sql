...
create or replace function users_snap (ts timestamptz, id uuid default null) returns setof user_events language sql as $$
  select
    ts as timestamp,
...
create or replace function users_snap_commit (ts timestamptz, id uuid default null) returns setof user_events language sql as $$
  insert into user_events select * from users_snap(ts, users_snap_commit.id) returning *;
$$;

create or replace view users_latest_snaps as
select last(timestamp) as timestamp, stream_id, 'snap' as name, last(data) as data
from user_events where name = 'snap' group by stream_id;
...
select * from users_latest_events
union
select * from users_latest_snaps;
